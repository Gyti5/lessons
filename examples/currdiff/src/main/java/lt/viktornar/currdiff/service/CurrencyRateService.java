...
public class CurrencyRateService {
    ...
    public List<Item> getChangesOfRatesByDate(Date date) {
        ...
        if (!itemsOfGivenDate.isEmpty() && !itemsOfPrevDate.isEmpty()) {
            ...
            itemsOfGivenDate.forEach(gItem -> {

                itemsOfPrevDate.stream().filter(PItem -> gItem.getCurrency().equals(PItem.getCurrency())).forEach(PItem -> {
                    Item itemToReturn = new Item();
                    BeanUtils.copyProperties(gItem, itemToReturn);
                    Float diffRate = gItem.getRate() - PItem.getRate();
                    float percentage = (diffRate / PItem.getRate()) * 100f;
                    itemToReturn.setRateChangeInPercentage(roundRateChange(percentage));
                    itemToReturn.setRateChange(roundRateChange(diffRate));
                    results.add(itemToReturn);
                });
            });
